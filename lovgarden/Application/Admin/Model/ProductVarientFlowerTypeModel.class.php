...
class ProductVarientFlowerTypeModel extends Model 
{
    ...
    public function product_varient_id_to_flower_type_id_delete($product_varient_id,$delete_ids) {
        $result = TRUE;
    ...
    public function product_varient_id_to_flower_type_id_update($product_varient_id,$new_flower_type_ids=array()){
        //先查出原来$product_varient_id对应的flower_type_id保存到一个数组里
        $add_status = TRUE;
        $delete_status = TRUE;
        $sql = "SELECT flower_type_id FROM lovgarden_product_varient_flower_type WHERE product_varient_id = '$product_varient_id'";
        $old_type_ids = $this->query($sql);
        //整理组织查出来的数据，便于比对
        $old_type_ids_fix = array(); 
        foreach($old_type_ids as $key => $value) {
            $old_type_ids_fix[] = $value['flower_type_id'];
        }
        //交叉比对这次提交上来的数据和老的数据
        ...
        foreach($new_flower_type_ids as $key => $value) {
            if(!in_array($value, $old_type_ids_fix)) {
                //说明是要新增的flower_type_id
                $array_add_ones[] = $value;
            }
        }
        foreach($old_type_ids_fix as $key => $value) {
            if(!in_array($value, $new_flower_type_ids)) {
                //说明是要删除的flower_type_id
                $array_delete_ones[] = $value;
            }
        }     
        if(!empty($array_add_ones)) {
            $add_status = $this->product_varient_id_to_flower_type_id_add($product_varient_id, $array_add_ones);
        }
        if(!empty($array_delete_ones)) {
            $delete_status = $this->product_varient_id_to_flower_type_id_delete($product_varient_id,$array_delete_ones);
        }
